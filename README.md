# 지식의 집

브라우저에서 직접 글을 쓰고 발행하는 개인 지식 저장소입니다.
서버 없이 **GitHub 저장소를 데이터베이스처럼** 사용합니다.

```
글 작성 → GitHub API로 커밋 → Actions가 색인 생성 → Pages 배포
```

## 처음 설정

### 1. GitHub Pages 켜기

저장소 **Settings → Pages** 에서 **Source** 를 `GitHub Actions` 로 선택합니다.

### 2. 토큰 발급

사이트에 접속해 우측 상단 **⚙ 설정** 을 열면 안내가 나옵니다. 요약하면:

1. GitHub → Settings → Developer settings
2. Personal access tokens → **Fine-grained tokens** → Generate new token
3. **Repository access** → *Only select repositories* → 이 저장소만 선택
4. **Permissions** → Repository permissions → **Contents** 를 *Read and write* 로
5. 생성된 토큰을 사이트 설정 창에 붙여넣기

토큰은 브라우저의 `localStorage` 에만 저장되며 저장소에 올라가지 않습니다.
따라서 사이트를 방문한 다른 사람은 글을 쓰거나 고칠 수 없습니다.

> ⚠️ 공용 컴퓨터에서는 토큰을 저장하지 마세요. 유출이 의심되면 GitHub 설정에서 즉시 무효화할 수 있습니다.

### 3. 토큰 유지

한 번 등록하면 계속 유지됩니다. 매번 입력해야 한다면 아래 중 하나가 원인입니다.
설정 창의 **"토큰이 자꾸 풀린다면"** 항목에도 같은 안내가 있습니다.

| 원인 | 해결 |
| --- | --- |
| 사파리 / 아이폰에서 7일 이상 미방문 | WebKit 이 저장 데이터를 지웁니다. 홈 화면에 추가해두고 가끔 열어주세요 |
| 시크릿(사생활 보호) 창 | 일반 창에서 사용하세요. 시크릿 창은 저장이 아예 안 됩니다 |
| 브라우저 "종료 시 데이터 삭제" 설정 | 이 사이트를 예외로 등록하세요 |
| 토큰 만료 | 설정 창 상단에 만료일이 표시됩니다. 발급 시 기간을 길게 잡으세요 |
| 다른 기기 · 다른 브라우저 | 기기마다 한 번씩 등록해야 합니다 |

도움이 되도록 다음을 넣어두었습니다.

- 저장이 실제로 됐는지 **되읽어 확인** — 조용히 실패하는 사파리 시크릿 모드를 감지해 알려줍니다
- `navigator.storage.persist()` 로 **저장 데이터를 지우지 말라고 요청**
- 토큰 **만료일 표시**와 만료 7일 전 알림 (GitHub 응답 헤더에서 읽음)
- 설정 창이 **form** 이므로 브라우저·비밀번호 관리자가 토큰을 기억해 한 번에 다시 채워줍니다
- 헤더의 **⚙ 버튼에 점 표시** — 토큰이 없거나 만료가 임박하면 눈에 띕니다

## 구조

| 경로 | 설명 |
| --- | --- |
| `index.html` | 글 목록 (검색, 태그 필터) |
| `write.html` | 에디터 (슬래시 서식 메뉴, 이미지 붙여넣기) |
| `post.html` | 글 보기 (수정, 삭제, 변경 기록) |
| `js/folders.js` | 폴더 구조 계산 (경로 다듬기, 나무 만들기, 정렬) |
| `js/docs.js` | 문서 종류 정의 (정리 · 요약본 · 원문) |
| `posts/` | 발행된 글 (마크다운) |
| `summaries/` | 글마다 짝이 되는 요약본 |
| `originals/` | 글마다 짝이 되는 원문 |
| `drafts/` | 초안 |
| `assets/images/` | 업로드된 이미지 |
| `data/posts.json` | 목록 색인 — Actions가 배포 시 생성 (커밋되지 않음) |
| `scripts/build-index.mjs` | 색인 생성 스크립트 |
| `config.js` | 사이트 이름, 대상 저장소 설정 |

## 다른 곳에서 복사해 붙여넣기

에디터에 붙여넣으면 서식을 살려 마크다운으로 바꿔줍니다.

| 원본 | 결과 |
| --- | --- |
| 제목 (`<h1>`~`<h6>`) | `#` ~ `####` |
| 굵게 / 기울임 / 취소선 | `**`, `*`, `~~` |
| 목록 · 번호 목록 · 중첩 · 체크박스 | 마크다운 목록 |
| 인용 · 코드블록 · 표 · 가로선 | 각각의 마크다운 문법 |
| 링크 · 이미지 | `[글자](주소)`, `![설명](주소)` |

노션, 웹페이지, 워드, 구글 문서에서 복사한 경우를 모두 다룹니다.
구글 문서가 붙이는 `<b style="font-weight:normal">` 같은 껍데기, 줄바꿈 안 되는 공백(NBSP),
보이지 않는 문자 등 붙여넣기 찌꺼기도 함께 정리합니다.

**줄바꿈은 쓴 대로 보입니다.** 한 번 누른 Enter 도 화면에서 줄이 바뀝니다.
(일반 마크다운은 한 줄 띄움을 무시하고 문단을 이어붙이지만, 글쓰기 편의를 위해 그렇게 하지 않습니다)

### 정리 버튼

번호가 붙은 소제목을 마크다운 제목으로 바꿔줍니다.

```
1. 문제 인식 — 우리는 쳇바퀴를 돌리고 있다   →   ## 1. 문제 인식 — 우리는 쳇바퀴를 돌리고 있다
2-1. 우리는 하루 종일 자신과 대화한다        →   ### 2-1. 우리는 하루 종일 자신과 대화한다
```

위아래에 같은 모양의 번호 줄이 붙어 있으면 **진짜 목록으로 보고 건드리지 않습니다.**
`Ctrl`+`Z` 로 되돌릴 수 있습니다.

## 폴더

글을 폴더로 묶어 정리합니다. **파일을 옮기지 않고 메타데이터로 관리**합니다.

```markdown
---
title: 챕터1
folder: 책1          ← 이 한 줄이 폴더
---
```

`/` 로 나누면 폴더 안에 폴더가 됩니다. 깊이 제한은 없습니다.

```
folder: 독서/책1/1부
```

파일 경로가 그대로이므로 **글을 다른 폴더로 옮겨도 주소가 바뀌지 않고**, 원문 짝도 유지됩니다. 폴더 이름을 바꿀 때도 파일을 옮길 필요가 없습니다.

### 화면

| 주소 | 보이는 것 |
| --- | --- |
| `index.html` | 최상위 폴더 카드만 |
| `index.html?folder=책1` | 책1 안의 하위 폴더 + 글 |
| `index.html?folder=__unfiled__` | 폴더를 정하지 않은 글 |
| `index.html?tag=독서` | 태그로 걸러진 글 |

- 폴더를 미리 만들 필요가 없습니다. **글을 쓰면서 폴더 이름을 적으면 그 폴더가 생깁니다.**
- 에디터의 폴더 칸에 기존 폴더가 **자동완성**으로 뜹니다
- 폴더 안에서 **글쓰기**를 누르면 그 폴더가 미리 채워집니다
- 검색은 폴더와 상관없이 **전체에서** 찾습니다
- 폴더를 정하지 않은 글은 **미분류** 에 모입니다 (사라지지 않습니다)

### 글 순서

폴더 안에서 정렬 방식을 자동으로 고릅니다.

| 조건 | 정렬 |
| --- | --- |
| 제목에 숫자가 든 글이 둘 이상 (연재물) | **순서대로** — `챕터1 → 챕터2 → 챕터10` |
| 그 밖 | **최신순** |

`챕터2` 가 `챕터10` 보다 앞에 오도록 숫자를 숫자로 비교합니다.
목록 오른쪽의 **순서대로 / 최신순** 버튼으로 언제든 바꿀 수 있습니다.

## 딸린 문서 — 요약본 · 원문

글 하나는 **세 개의 문서**를 가질 수 있습니다. 책이나 기사를 정리한 글에서 요약과 원본을 함께 보관하는 용도입니다.

```
posts/2026-09-03-챕터1.md      정리
summaries/2026-09-03-챕터1.md  요약본   ← 파일명이 같아서 짝이 된다
originals/2026-09-03-챕터1.md  원문
```

글을 열면 제목 아래에 **정리 / 요약본 / 원문** 전환 탭이 나옵니다.

- 있는 문서는 누구나 전환해 볼 수 있습니다
- 없는 문서는 글쓴이에게만 **＋ 요약본**, **＋ 원문** 으로 보입니다
- 세 문서 모두 똑같이 쓰고, 고치고, 지울 수 있습니다 (변경 기록도 각각 남습니다)
- 목록에서는 `요약본` `원문` 표시가 붙습니다

> ⚠️ **딸린 문서의 파일명은 고정입니다.** 정리본과 이름이 같아야 짝이 유지되므로, 요약본·원문에서 제목을 바꿔도 파일명은 바뀌지 않습니다. 정리본의 제목을 바꾸면 정리본 파일명이 변경되어 짝이 끊기므로, 그때는 다시 연결해야 합니다.

### 종류를 더 늘리려면

`js/docs.js` 의 `KINDS` 에 한 줄, `config.js` 에 폴더 이름, `scripts/build-index.mjs` 의 `COMPANIONS` 에 한 줄만 추가하면 됩니다. 화면과 에디터는 이 목록을 따라 자동으로 늘어납니다.

```js
const KINDS = [
  { key: 'main', label: '정리', dirKey: 'postsDir' },
  { key: 'summary', label: '요약본', dirKey: 'summariesDir' },
  { key: 'original', label: '원문', dirKey: 'originalsDir' },
];
```

## 글 파일 형식

```markdown
---
title: 글 제목
date: 2026-09-03
folder: 책1
tags: [태그1, 태그2]
status: 자라는 중
---

본문을 마크다운으로 씁니다.
```

## 단축키

| 단축키 | 하는 일 |
| --- | --- |
| `/` | 에디터에서 서식 메뉴 열기 |
| `Ctrl`/`Cmd` + `S` | 초안 저장 |
| `Ctrl`/`Cmd` + `Enter` | 발행 |
| `Ctrl`/`Cmd` + `Shift` + `P` | 미리보기 전환 |
| `Ctrl`/`Cmd` + `K` | 목록에서 검색창으로 이동 |

## 로컬에서 확인하기

`file://` 로 열면 브라우저 보안 정책 때문에 목록을 읽지 못합니다. 간단한 서버를 띄워주세요.

```bash
python3 -m http.server 8000
# http://localhost:8000
```

## 설정 바꾸기

`config.js` 에서 사이트 이름, 소개 문구, 대상 저장소를 바꿀 수 있습니다.

```js
window.SITE_CONFIG = {
  siteName: '지식의 집',
  tagline: '읽고 생각한 것을 쌓아두는 곳',
  owner: 'aicatveo3-prog',
  repo: 'knowledge-house',
  branch: 'main',
  ...
};
```
